...
transition game s r = case (stateValue s, stateData s, r) of
  ...
  (v, GameDatum bs (Just c), Reveal _ choice)
    | lovelaces v == (2 * gStake game) && beats choice c ->
      Just
        ( Constraints.mustBeSignedBy (gFirst game)
            <> Constraints.mustValidateIn (to $ gRevealDeadline game),
          State Finished mempty
        )
    | lovelaces v == (2 * gStake game) && choice == c ->
      Just
        ( Constraints.mustBeSignedBy (gFirst game)
            <> Constraints.mustValidateIn (to $ gRevealDeadline game),
...
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just _)) (Reveal nonce choice) _ =
  sha2_256
    ( nonce
        `concatenate` if choice == Rock then bsRock' else if choice == Scissors then bsScissors' else bsPaper'
    )
    == bs
...
firstGame fp = do
  ...
  let game =
      ...
      bs =
        sha2_256 $
          fpNonce fp
            `concatenate` if c == Rock then bsRock else if c == Scissors then bsScissors else bsPaper
    ...
    Just ((o, _), _) -> case tyTxOutData o of
      GameDatum _ Nothing -> do
        ...
        case m' of
          ...
          Just _ -> do
            logInfo @String "first player didn't reveal or draw"
            void $ mapError' $ runStep client ClaimSecond
            logInfo @String "second player won or draw"
